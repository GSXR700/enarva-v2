...
export function LeadForm({ lead, onSuccess, onCancel, users = [] }: LeadFormProps) {
  const [isLoading, setIsLoading] = useState(false);
  const [localUsers, setLocalUsers] = useState<Array<{id: string, name: string}>>([]);
  ...
  // Load users if not provided via props
  useEffect(() => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (Array.isArray(userList)) {
              setLocalUsers(userList.map((user: any) => ({
                id: user.id,
  ...
  // Use localUsers if users prop is empty, otherwise use users prop
  const usersToRender = users && users.length > 0 ? users : localUsers;
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    {Array.isArray(usersToRender) && usersToRender.map((user) => (
