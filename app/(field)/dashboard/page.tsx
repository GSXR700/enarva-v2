  ...
  const startMission = async (missionId: string) => {
    try {
      const response = await fetch(`/api/missions/${missionId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ status: 'IN_PROGRESS', actualStartTime: new Date().toISOString() })
      })
                                      ...
                                      <DropdownMenuContent align="start" className="w-48">
                                        {selectedMission.team.members.map((member) => (
                                          <DropdownMenuItem key={member.id} onClick={() => handleAssignTask(task.id, member.id)} className="flex items-center gap-2 cursor-pointer">
                                            ...
                                            {task.assignedTo?.user.id === member.user.id && (
                    ...
                    <TabsContent value="actions" className="space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {selectedMission.status === 'SCHEDULED' && isTeamLeader && (
                          <Button className="w-full" onClick={() => startMission(selectedMission.id)}>
                            <Play className="h-4 w-4 mr-2" />
                            Démarrer la mission
                          </Button>
                        )}
                        <Button variant="outline" className="w-full" asChild>
                          <a href={`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(selectedMission.address)}`} target="_blank" rel="noopener noreferrer">
